...
static const float MUD_STRENGHTEN 				= 0.2;

static const float 	SNOW_OPACITY_MIN			= 0.4;
static const float 	SNOW_OPACITY_MAX			= 0.8f;
static const float 	SNOW_CAM_MIN 				= 50.0f;
static const float 	SNOW_CAM_MAX 				= 400.0f;
static const float 	MUD_CAM_MIN 				= 50.0f;
static const float 	MUD_CAM_MAX 				= 600.0f;
static const float 	ICE_CAM_MIN 				= 100.0f;
static const float 	ICE_CAM_MAX 				= 350.0f;


static const float 	SNOW_START_HEIGHT 			= 0.5f;
static const float 	SNOW_RIDGE_START_HEIGHT 	= 1.0f;
static const float 	SNOW_NORMAL_START 			= 0.1f;
static const float3 SNOW_COLOR 					= float3( 0.99, 0.99, 0.99 );
...
static const float WATER_RIPPLE_EFFECT = 0.00001;

static const float COLORMAP_OVERLAY_STRENGTH 	= 0.1f;
static const float COLORMAP_MUD_OVERLAY_STRENGTH = 0.01f;
...
// Fog of war
static const float 	FOW_MAX 					= 0.3f;
...
static const float  SHADOW_WEIGHT_BORDER   		= 0.1f;
static const float  SHADOW_WEIGHT_WATER   		= 0.1f;
static const float  SHADOW_WEIGHT_RIVER   		= 0.1f;
static const float  SHADOW_WEIGHT_TREE   		= 0.1f;
...
static const float GB_CAM_MIN = 50.0f;
static const float GB_CAM_MAX = 250.0f;
static const float GB_CAM_MAX_FILLING_CLAMP = 0.7f; // 0 to 1 value for clamping the fill when camera is at max distance
...
static const float GB_OPACITY_NEAR = 0.33f; // Transparency when camera is near
static const float GB_OPACITY_FAR = 0.99f;  // Transparency when camera is far
static const float BORDER_NIGHT_DESATURATION_MAX = 0.6f; // how much border colors can get desaturated at night. 1.0f is full grey
static const float BORDER_FOW_REMOVAL_FACTOR = 0.7f; // How much of the FOW that is removed from the borders. 1.0f is no FOW
...
static const float GB_STRENGTH_CH1 = 0.90f; // Opacity of bottom layer
static const float GB_STRENGTH_CH2 = 0.90f; // Opacity of top layer
static const float GB_FIRST_LAYER_PRIORITY = 0.1f; // Priority for first/second layer when both are active at the same pixel
...
static const float MAP_ARROW_NORMALS_STR_TERR = 0.025f;
static const float MAP_ARROW_NORMALS_STR_WATER = 0.04f;
