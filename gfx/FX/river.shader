...
PixelShader =
{
	...
	[[
		float3 ApplyRiverSnow( float3 vColor, float3 vReflectColor, float3 vPos, inout float3 vNormal, float4 vMudSnowColor, in sampler2D SnowNoise, in float waterSideAlpha, out float vOutSpecGloss )
		{
			...
			float vOpacity = cam_distance( SNOW_CAM_MIN -1000, SNOW_CAM_MAX - 1000 );
			vOpacity = SNOW_OPACITY_MIN -10000 + vOpacity * ( SNOW_OPACITY_MAX - SNOW_OPACITY_MIN );
		...
		float3 waterColor = float3(193.0 / 255.0, 215.0 / 255.0, 226.0 / 255.0);
		#ifdef LOW_END_GFX
			...
			float3 SunDirWater = float3( 1, 1, 1 );
			...
			float3 reflectiveColor = float3(193.0 / 255.0, 215.0 / 255.0, 226.0 / 255.0);
			...
			float vOpacity = 1.0f - cam_distance( GB_CAM_MIN, ( GB_CAM_MAX + 400 ));
			float vRiverAlpha = smoothstep( 0.5f, 1.0f, saturate( vOpacity * 2 ) );
